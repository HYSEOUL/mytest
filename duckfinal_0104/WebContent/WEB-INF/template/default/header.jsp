...
@font-face {
	...
	src: url('fonts/NanumGothicRegular.eot?#iefix')
		format(‘embedded-opentype’), url('fonts/NanumGothicRegular.woff')
		format(‘woff’), url('fonts/NanumGothicRegular.ttf') format('truetype');
	font-weight: normal;
	font-style: normal;
}
...
<div class="container text-center">
	...
	<c:if test="${not empty mt_id}">
		<p style="color: white; float: right;">${mt_id }강사 님환영합니다♥</p>
	</c:if>
	<c:if test="${not empty ms_id}">
		<p style="color: white; float: right;">${ms_id }학생 님환영합니다♥</p>
	</c:if>
	</div>
	<div class="container text-center"
      ...
      <!-- 강사와 학생의 session값이 없으면 가입 태그와 로그인 태그가 생성 -->
       ...
       <!-- 학생 session값이 있으면 마이페이지 로그인 태그가 생성 -->
		...
		<!-- 강사 session값이 있으면 강사페이지 로그인 태그가 생성 -->
				...
				<!-- <li class="nav-item"><a class="nav-link" href="chatmain">실시간 강의정보공유</a>
				</li> -->
