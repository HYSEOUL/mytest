...
* triggerSelectEvent() function이 내가 주로 핸들링한 달력의 기능임. 달력찍으면 나오는 데이터의 모든것
* 그 외에 질문은 예비군이 도망친걸로 마무리 해줘...
*/
(function ($) {
	...
	function selectMonth(next, options, month, year) {
		...
		var tmpYear = parseInt(tmp[1], 10);
		
		currentMonth = 
			month || ((next) ? ((tmp[0] === options.months[options.months.length - 1]) ? 0 : options.months.indexOf(tmp[0]) + 1) : ((tmp[0] === options.months[0]) ? 11 : options.months.indexOf(tmp[0]) - 1));
	...
	function triggerSelectEvent(event) {
		...
		label[1] = ((date.getMonth() + 1) < 10) ? '0' +
				(date.getMonth() + 1) : date.getMonth() + 1;
		label[2] = (date.getDate() < 10) ? '0' +
